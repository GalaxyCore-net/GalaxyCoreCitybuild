...
public class SignCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        itemMeta.lore(Collections.singletonList(Component.text(StringUtils.replaceRelevant(I18N.getByPlayer(player, "citybuild.sign.text"), new LuckPermsApiWrapper(player)).replaceAll("%time%", new SimpleDateFormat(
                I18N.getInstanceRef().get().getLanguages()
                        .get(I18N.getInstanceRef().get().getLocale(player)).getDateFormat())
