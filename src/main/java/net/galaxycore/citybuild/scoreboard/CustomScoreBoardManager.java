...
public class CustomScoreBoardManager implements IScoreBoardCallback {
    ...
    public String[] getKV(Player player, int id) {
        ...
        OnlineTime onlineTime = OnlineTime.getOnlimeTime(player);
        if (id == 0)
        ...
        if (id == 2)
            kv = new String[]{I18N.getByPlayer(player, "citybuild.score.onlinetime"), "§7" + I18N.getByPlayer(player, "citybuild.score.onlinetime.sub").replace("%h%", (int) Math.floor(onlineTime.getHours()) + "").replace("%m%", (int) Math.floor(onlineTime.getMinutes()) + ""), ""};
        if (id == 3)
