...
@Getter
public class LongFlagMenu extends Menu {
    private final Player player;
    private final Plot plot;
    private final Flag flag;
    private final Class<? extends PlotFlag<Long, ?>> flagClass;

    public LongFlagMenu(Player player, Plot plot, Flag flag) {
        super(PlayerMenuUtility.getPlayerMenuUtility(player));
        this.player = player;
        this.plot = plot;
        this.flag = flag;
        //noinspection unchecked
        flagClass = (Class<? extends PlotFlag<Long, ?>>) flag.getFlagClass();
    }
    ...
    public void handleMenu(InventoryClickEvent inventoryClickEvent) {
        Long value = plot.getFlag(flagClass);
                    ...
                    new AnvilGUI.Builder()
                            ...
                            .onComplete((player1, s) -> {
                                if (s.equals(PMenuI18N.PLOTINFO_FLAGS_NONE.get(player))) {
                                    ...
                                    try {
                                        plot.setFlag(GlobalFlagContainer.getInstance().getFlag(flagClass).createFlagInstance(Long.valueOf(s)));
    ...
    public void setMenuItems() {
        @NonNull Long value = plot.getFlag(flagClass);

        inventory.setItem(11, makeItem(Material.BARRIER, PMenuI18N.RESET.get(player)));
        inventory.setItem(13, makeItem(Material.SPRUCE_SIGN, PMenuI18N.FLAGS_CURRENTVAL.get(player) + (value%24000)));
