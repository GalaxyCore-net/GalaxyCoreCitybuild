...
public class InvToggleCommand implements CommandExecutor {
    @SneakyThrows
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        Player player = (Player) sender;
        if (!player.hasPermission("citybuild.command.invtoggle")) {
        ...
        PreparedStatement statementUpdatePlayer = Essential.getCore().getDatabaseConfiguration().getConnection().prepareStatement("UPDATE galaxycity_playerdb SET invtoggle = ? where ID = ?");
        ...
        if (resultIsPlayerLocked.next()) {
            if (resultIsPlayerLocked.getBoolean("invtoggle")) {
                ...
                player.sendMessage(I18N.getByPlayer(player, "citybuild.inventar.close"));
            } else {
                statementUpdatePlayer.setBoolean(1, true);
                player.sendMessage(I18N.getByPlayer(player, "citybuild.inventar.open"));
            }
        }
