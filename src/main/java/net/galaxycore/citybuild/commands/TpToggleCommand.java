...
public class TpToggleCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!player.hasPermission("citybuild.command.tptoggle")) {
        ...
        PreparedStatement statementUpdatePlayer = Essential.getCore().getDatabaseConfiguration().getConnection().prepareStatement("UPDATE galaxycity_playerdb SET tptoggle = ? where ID = ?");
        ...
        if (resultIsPlayerLocked.next()) {
            if (resultIsPlayerLocked.getBoolean("tptoggle")) {
                ...
                player.sendMessage(I18N.getByPlayer(player, "citybuild.tptoggle.close"));
            } else {
                statementUpdatePlayer.setBoolean(1, true);
                player.sendMessage(I18N.getByPlayer(player, "citybuild.tptoggle.open"));
            }
        }
