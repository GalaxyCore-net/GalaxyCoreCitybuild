...
public class CustomScoreBoardManager implements IScoreBoardCallback {
    ...
    public String[] getKV(Player player, int id) {
        ...
        if (id == 2)
            kv = new String[]{I18N.getByPlayer(player, "citybuild.score.onlinetime"), "§7" + I18N.getByPlayer(player, "citybuild.score.onlinetime.sub").replace("%h%", Math.floor(onlineTime.getHours()) + "").replace("%m%", Math.floor(onlineTime.getMinutes()) + ""), ""};
        if (id == 3)
